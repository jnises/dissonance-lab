...
mod tests {
    ...
    fn test_impulse_response() {
        ...
        assert!(impulse_response.abs() > 0.0, "Impulse should produce immediate output");
        
        // The dry component should be around 0.4 (dry_level * input)
        let expected_dry = 0.4; // DEFAULT_DRY_LEVEL * 1.0
        assert!(impulse_response >= expected_dry * 0.9, "Should include significant dry component");
    ...
    fn test_dry_wet_mix() {
        ...
        assert!((dry_only - 0.5).abs() < 0.01, "Dry-only should pass input through");
        ...
        assert!(has_wet_output, "Wet-only should produce some output after delays");
    }
    ...
    fn test_room_size_effect() {
        ...
        assert!(large_energy > small_energy * 1.1, 
    ...
    fn test_damping_effect() {
        ...
        if low_damp_hf_energy > 0.001 && high_damp_hf_energy > 0.001 {
            assert!(low_damp_hf_energy > high_damp_hf_energy * 1.1,
    ...
    fn test_parameter_clamping() {
        ...
        assert!(output.is_finite(), "Reverb should produce finite output even with invalid parameters");
    }
    ...
    fn test_silence_input() {
        ...
        assert_eq!(output, 0.0, "Silence input should produce silence output initially");
        
        // Continue processing silence
        for _ in 0..1000 {
            let sample = reverb.process(0.0);
            assert!(sample.abs() < 1e-10, "Continued silence should remain silent");
        }
    }
    ...
    fn test_stability() {
        ...
        for &input in &test_inputs {
            for _ in 0..1000 {
                let output = reverb.process(input);
                assert!(output.is_finite(), "Output should always be finite for input {input}");
                assert!(output.abs() < 100.0, "Output should not grow unbounded for input {input}");
            }
        }
    }
}
